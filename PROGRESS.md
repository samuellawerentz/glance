...
### Phase 3 (manual)
- [x] `m-setup-script-idempotent` · P1 · manual — EXECUTED 2026-06-25 against real CF account (samuel.lawerence@plivo.com). Re-run keeps SESSION_SECRET + CONTENT_TOKEN_SECRET on both workers, leaves BOOTSTRAP_TOKEN, "No migrations to apply", skips URL wiring (sentinel already consumed). Idempotent confirmed.
- [x] `m-one-click-fresh-deploy` · P0 · manual — EXECUTED 2026-06-25. `setup.sh` deployed both workers to `glance.samuel-lawerence.workers.dev`. Live verified: `/api/config` returns real shape, cross-origin bootstrap POST → 403 (CSRF), same-origin valid token → 200 + Secure HttpOnly session cookie. Note: account had prior state (Google creds + existing superadmin), so the 200 was the idempotent same-superadmin path, not a virgin insert.
...
- 2026-06-25 — **Phase 3 manual smoke EXECUTED** against a real Cloudflare account (personal, samuel.lawerence@plivo.com). Both `m-one-click-fresh-deploy` + `m-setup-script-idempotent` now `[x]`. Live at `glance.samuel-lawerence.workers.dev`. Closes the two outstanding `[~]` items — feature fully verified end-to-end on real infra.
